...
Agent::Position agent_A::choose_new_position(const Map& map) const {
    Position p(0,0);
    ...
    bool ok = false;
    int i = limita_min.row;
    int j;
    // verific daca se afla un alt agent in perimetru
    while (ok == false && i <= limita_max.row) {
        ...
        while(ok == false && j <= limita_max.col) {
            ...
            if(p.row >= 0 && p.row < n && p.col >= 0 && p.col < m && !(p == position))
                if(map.get_agent_position(p.row,p.col) != NULL)
                    ok = true;
            j++;
        }
        i++;
    }
    int x[2] = {1,-1};//Acest tip de agent se deplaseaza cu maxim o                  //patratica pe runda
    int y[2] = {1,-1};
...
//incercare esuata 
/*Agent::Position agent_A::choose_new_new_position(const Map& map) const {
    Position p(0,0);
    int ok=false;
    int x[2] = {1,-1};
    int y[2] = {1,-1};
